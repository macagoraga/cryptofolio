...
$(function() {
    ...
    $(".item").on("click", function(event) {
        ...
        $(".1d").on("click", function(event) {
            clearInterval(graphrefresh);
            loadgraph(s, 'today', 1440);
        });
...
function portfolio() {
    ...
    $.ajax({
        ...
        success: function(json) {
           ...
           var out = json.RAW
           $("#euro").text(btceuro);
           $("#btceuro").text("BTC "+formatter(btceuro,'n'));
           $("#etheuro").text("ETH "+formatter(etheuro,'n'));
           $("#ltceuro").text("LTC "+formatter(ltceuro,'n'));

           Object.keys(out).forEach(function(key,index) {
                price = (out[key].BTC.PRICE*btceuro);
